...
*/
;(function() {
    const PLATFORM_VERSION_BUILD_LABEL = '9.0.0';
    ...
    (function () {
        ...
        function build(module) {
            ...
            const localRequire = function (id) {
                let resultantId = id;
                // It's a relative path, so lop off the last portion and add the id (minus "./")
                if (id.charAt(0) === '.') {
                    resultantId =
...
define("cordova/android/nativeapiprovider", function(require, exports, module) {
     ...
     */
    const nativeApi = this._cordovaNative || require('cordova/android/promptbasednativeapi');
...
define("cordova/exec", function(require, exports, module) {
    ...
    let messagesFromNative = [];
    let isProcessing = false;
    let resolvedPromise = typeof Promise === 'undefined' ? null : Promise.resolve();
    let nextTick = resolvedPromise ? function (fn) { resolvedPromise.then(fn); } : function (fn) { setTimeout(fn); };
...
function androidExec (success, fail, service, action, args) {
    ...
    const msgs = nativeApiProvider.get().exec(bridgeSecret, service, action, callbackId, argsJson);
...
function pollOnce (opt_fromOnlineEvent) {
    ...
    const msgs = nativeApiProvider.get().retrieveJsMessages(bridgeSecret, !!opt_fromOnlineEvent);
    if (msgs) {
        messagesFromNative.push(msgs);
...
function buildPayload (payload, message) {
    ...
    if (payloadKind === 's') {
        ...
        while (multipartMessages !== '') {
            ...
            const multipartMessage = multipartMessages.substring(spaceIdx + 1, msgLen);
...
function popMessageFromQueue () {
    ...
    const message = messageBatch.substring(spaceIdx + 1, msgLen);
...
define("cordova/init", function(require, exports, module) {
    ...
    const platformInitChannelsArray = [channel.onNativeReady, channel.onPluginsReady];
...
define("cordova/modulemapper", function(require, exports, module) {
    ...
    const moduleMap = define.moduleMap;
    let symbolList;
    let deprecationMap;
...
exports.mapModules = function (context) {
    ...
    for (let i = 0, len = symbolList.length; i < len; i += 3) {
        ...
        const namespace = symbolPath.substring(0, lastDot);
        const lastName = symbolPath.substring(lastDot + 1);

        const deprecationMsg = symbolPath in deprecationMap ? 'Access made to deprecated symbol: ' + symbolPath + '. ' + deprecationMsg : null;
...
// The last resume event that was received that had the result of a plugin call.
    ...
    bootstrap: function () {
        ...
        const APP_PLUGIN_NAME = Number(cordova.platformVersion.split('.')[0]) >= 4 ? 'CoreAndroid' : 'App';
        ...
        function bindButtonChannel (buttonName) {
            ...
            const volumeButtonChannel = cordova.addDocumentEventHandler(buttonName + 'button');
...
define("cordova/plugin/android/app", function(require, exports, module) {
    ...
    const exec = require('cordova/exec');
    const APP_PLUGIN_NAME = Number(require('cordova').platformVersion.split('.')[0]) >= 4 ? 'CoreAndroid' : 'App';
